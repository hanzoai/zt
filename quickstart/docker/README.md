...
1. run the script `./createLocalImage.sh` which will create a `hanzozt/quickstart:latest` tag
   using the latest `ziti` [release from GitHub](https://github.com/hanzozt/ziti/releases/latest)
...
1. run the script `ZITI_VERSION_OVERRIDE=0.29.0 ./createLocalImage.sh` which will create a `hanzozt/quickstart:latest` tag
   using the specified version of `ziti` [from GitHub](https://github.com/hanzozt/ziti/releases/tag/v0.29.0)
...
To publish the latest `ziti` CLI, use [the GitHub Action](https://github.com/hanzozt/ziti/actions/workflows/push-quickstart.yml).
