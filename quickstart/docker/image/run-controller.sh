#!/bin/bash
zt_controller_cfg="${ZITI_HOME}/zt-edge-controller.yaml"
...
if [ ! -f "${ZITI_HOME}/access-control.init" ]; then
  ...
  # create pki
  createPki
  if [ ! -f "${zt_controller_cfg}" ]; then
    echo " "
    echo "${zt_controller_cfg} doesn't exist. Generating config file"
    echo " "
    # generates the config file for the controller
    createControllerConfig
  else
    echo " "
    echo "${zt_controller_cfg} exists. Not overwriting"
    echo " "
  fi

  # initialize the database with the admin user:
  "${ZITI_BIN_DIR}/zt" controller edge init "${ZITI_HOME}/${ZITI_CTRL_NAME}.yaml" -u "${ZITI_USER}" -p "${ZITI_PWD}"
  if [[ "$?" != 0 ]]; then
...
"${ZITI_BIN_DIR}/zt" controller run ${ZITI_VERBOSE:+--verbose} "${ZITI_HOME}/${ZITI_CTRL_NAME}.yaml"
