...
if [ -z "${ZITI_VERSION:-}" ]; then
  ...
  v=$(source "${DOCKER_IMAGE_ROOT}/zt-cli-functions.sh"; getLatestZitiVersion > /dev/null 2>&1; echo ${ZITI_BINARIES_VERSION})
...
docker buildx create \
  --use --name=zt-builder --driver docker-container 2>/dev/null \
  || docker buildx use --default zt-builder
