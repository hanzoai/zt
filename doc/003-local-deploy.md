...
- `zt controller` with the provided demo certificate authority in `./etc/ca`
- `zt router` as an edge router
...
Also note that the `zt` command may need to have the `.exe` suffix appended to the example commands.
...
ZITI_PKI_SIGNER_KEY=./etc/ca/intermediate/private/intermediate.key.decrypted.pem \
    zt create config controller \
        --output ./db/ctrl-config.yml
```

```bash
zt controller edge init ./db/ctrl-config.yml -u ADMIN_NAME -p ADMIN_PW
```
...
```bash
zt controller run ./db/ctrl-config.yml
```

## Login to the Controller

You will need a new terminal with current directory set to the top-level of this repo.This login step will save a session token in the `zt` CLI's configuration cache.

```bash
zt edge login -u ADMIN_NAME -p ADMIN_PW
```

Subsequent `zt` CLI commands will automatically reuse this session token. You'll need to perform this login step again when the token expires.
...
```bash
zt edge create edge-router router01 \
...
```bash
zt router enroll --jwt /tmp/router01.jwt ./db/router01-config.yml
```
...
```bash
zt router run ./db/router01-config.yml
```

## Create Your First Service

A service is an entity that stores metadata about a server application. The `zt` CLI has an interactive tutorial to step you through creating your first service.

When prompted, select your running edge router `router01`.

```bash
zt demo first-service
```

If you prefer, you may read [the first-service tutorial as a web site](../zt/cmd/demo/tutorials/first-service.md)
