...
This tutorial covers composing the controller PKI and minimal controller and router configs from scratch. [The previous tutorial](./003-local-deploy.md) guided you to run a controller with the demo PKI checked in to this repo under `./etc/ca` and with annotated and complete controller and router configs generated with the `zt create config` command.
...
- up-and-running-ctrl.zt.netfoundry.io
- up-and-running-er01.zt.netfoundry.io
- up-and-running-r01.zt.netfoundry.io
...
- Linux: `/etc/hosts`
    ...
    127.0.0.1   up-and-running-ctrl.zt.netfoundry.io
    127.0.0.1   up-and-running-er01.zt.netfoundry.io
    127.0.0.1   up-and-running-r01.zt.netfoundry.io
...
1. Generate the base directory structure and `env` file
    ...
    export ZITI_CTRL_HOSTNAME="${ZITI_NETWORK}-ctrl.zt.netfoundry.io"
    export ZITI_ER01_HOSTNAME="${ZITI_NETWORK}-er01.zt.netfoundry.io"
    export ZITI_R01_HOSTNAME="${ZITI_NETWORK}-r01.zt.netfoundry.io"
    ...
    Source the newly created `env` file: `source ${ZITI_HOME}/env` or if the defaults were used `source .zt/env`
    ...
    - `ZITI_PKI` - The location of the PKI used by `zt pki` via the `--pki-root` option
    ...
    - `ZITI_CA_FILE` - Location of the CA signing cert for ease of use using `zt` CLI commands
    ...
    - `ZITI_EDGE_API_HOSTNAME` - The hostname/port combination for the Ziti Edge API, used with `zt edge` commands

## Create A Certificate Authority

All communication between Ziti components is secured using mutual TLS. In order to get a zt environment running a certificate authority is required. This document guides the reader through creating a Root Certificate Authority (Root CA). It does not cover setups that should utilize Intermediate Certificate Authorities for production environments.

The `zt` CLI will be significantly streamline and reduce the prerequisite knowledge required to generate a Root CA and signed certificates for client and server usage. Specifically the `zt pki` command will do most of the heavy lifting.
...
1. Create the CA
    ...
    ```bash
    zt pki create ca --pki-root="${ZITI_PKI}" --ca-file="${ZITI_CA_NAME}"
    ```

    Additional certificate common name and subject fields can be set as needed. Run `zt pki create ca -h` for options.
...
1. Generate the controller Ziti Identity (server, client, and private key)
    ...
    Additional certificate common name and subject fields can be set as needed. Run `zt pki create server -h` or
     `zt pki create client -h` for options.
...
1. Initialize the database if this is the first time you are going to start the controller
    ...
    zt controller edge init $ZITI_HOME/controller.yaml -u admin -p admin
    
    # run the controller
    zt controller run $ZITI_HOME/controller.yaml
    ```
...
To enroll a Ziti Router, the command line utility `zt fabric` will be used that requires its own Ziti Identity in order
...
1. Authenticate w/ the controller if not authenticated
    ...
    zt edge login ${ZITI_EDGE_API_HOSTNAME} -u admin -p admin
    ```

1. Verify the identity works:

    ```bash
    zt fabric list routers
    ```
...
1. Enroll the Ziti Router identity with the controller
    ...
    ```bash
    zt fabric create router "${ZITI_PKI}/${ZITI_CA_NAME}/certs/${ZITI_NETWORK}-r01-client.cert"
    ```
...
1. Start the router:
    ...
    ```bash
    zt router run $ZITI_HOME/r01.yaml
    ```
...
enrollment process. Ziti Edge Routers and Ziti Routers are run from the same binary `zt router`.
...
1. Create a edge router and output the JWT to a file:
    ...
    ```bash
    zt edge create edge-router er01 cluster01 -o $ZITI_HOME/er01.jwt
    ```
...
1. Create the edge router configuration file
    ...
    edge:         
        ...
        sans:
            dns:
            - ${ZITI_NETWORK}-er01.zt.netfoundry.io
            ip:
            - "127.0.0.1"
    ...
    listeners:
        - binding: edge
          ...
          options:
            advertise: ${ZITI_NETWORK}-er01.zt.netfoundry.io:3022
        - binding: transport
...
1. Complete the enrollment process with the controller
    ...
    ```bash
    zt router enroll $ZITI_HOME/er01.yaml --jwt $ZITI_HOME/er01.jwt
    ```

1. Start the edge router:

    ```bash
    zt router run $ZITI_HOME/er01.yaml
    ```
