...
import (
	...
	"github.com/hanzozt/zt/v2/internal/rest/mgmt"
...
func (t *traffic) startServer(ctx context.Context, serviceName string, ztCfg *zt.Config) error {
	c, err := zt.NewContext(ztCfg)
	if err != nil {
		log.Fatal(err)
	}
...
func (t *traffic) startClient(client *rest_management_api_client.ZitiEdgeManagement, serviceName string, ztCfg *zt.Config) error {
	...
	ztReader := bufio.NewReader(svc)
	ztWriter := bufio.NewWriter(svc)

	text := "traffic test\n"
	bytesRead, err := ztWriter.WriteString(text)
	_ = ztWriter.Flush()
	if err != nil {
		log.Fatal(err)
	} else {
...
func enrollIdentity(client *rest_management_api_client.ZitiEdgeManagement, id string) *zt.Config {
...
func (t *traffic) configureServer() *zt.Config {
...
func (t *traffic) configureClient() *zt.Config {
