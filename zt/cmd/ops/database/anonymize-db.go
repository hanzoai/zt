...
func (self *anonymizeDbAction) run(_ *cobra.Command, args []string) {
	...
	ztDb, err := db.Open(dbFile)
	if err != nil {
		panic(err)
	}

	defer func() {
		if err = ztDb.Close(); err != nil {
			panic(err)
		}
	}()

	stores, err := db.InitStores(ztDb, command.NoOpRateLimiter{}, nil)
	if err != nil {
		panic(err)
	}
...
func (self *anonymizeDbAction) initValidation() {
	...
	err := self.ztDb.View(func(tx *bbolt.Tx) error {
...
func (self *anonymizeDbAction) anonymizeIdentities() {
	//  Update Identities
	err := self.ztDb.Update(nil, func(ctx boltz.MutateContext) error {
...
func (self *anonymizeDbAction) scrubSessions() {
	...
	err := self.ztDb.View(func(tx *bbolt.Tx) error {
		...
		if root == nil {
			return errors.New("root 'zt' bucket not found")
		}
		...
		if indexBucket == nil {
			logger.Info("zt index bucket does not exist, skipping indexes")
		} else {
