...
func newUpdateServiceEdgeRouterPolicyCmd(out io.Writer, errOut io.Writer) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Aliases: []string{"serp"},
		Short:   "updates a service edge router policy managed by the Hanzo ZT Edge Controller",
		Long:    "updates a service edge router policy managed by the Hanzo ZT Edge Controller",
