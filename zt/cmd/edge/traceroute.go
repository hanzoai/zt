...
func (o *traceRouteOptions) Run() error {
	var ctx zt.Context
	if o.configFile != "" {
		cfg, err := zt.NewConfigFromFile(o.configFile)
		if err != nil {
			return err
		}
	...
	if routerNameLookupsFailed {
		fmt.Println("Router name lookup failed. For this to work you must use zt edge login first and be an administrator.")
	}
	return nil
}
