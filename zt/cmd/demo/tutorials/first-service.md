...
Please note: this tutorial can be run interactively, by running 'zt demo first-service'. It can also be viewed as a web
page [here](https://github.com/hanzozt/zt/blob/release-next/zt/cmd/demo/tutorials/first-service.md). It may be convenient to be able to view both at the same time. The interactive version
...
you can use the quick-start script found [here](https://github.com/hanzozt/zt/tree/release-next/quickstart). The fastest way to invoke the quick-start is to run

`source <(wget -qO- https://raw.githubusercontent.com/hanzozt/zt/release-next/quickstart/docker/image/zt-cli-functions.sh); expressInstall`
...
NOTE: If you've used the quickstart, then your username and password can be found in `$HOME/.zt/quickstart/$(hostname)/$(hostname).env`
...
zt edge delete identities founder-laptop echo-server
zt edge delete service-policies where true
zt edge delete edge-router-policies where 'isSystem = false'
zt edge delete service-edge-router-policies where true
```
...
```action:zt templatize=true colorStdOut=false
zt demo plain-echo-client --port ${port} trees are tall
```
...
```action:zt templatize=true
zt edge create terminator echo ${edgeRouterName} tcp:localhost:${port}
```
...
```action:zt failOk=true
zt edge list terminators 'service.name="echo"'
```
...
See [Tunnelers](https://hanzozt.github.io/zt/clients/tunneler.html) for more information. Even though we're not covering tunnelers here, they are built with the Ziti SDKs. Therefore, all the
...
Let's take a look at our SDK client code (viewable [here](../zt_echo_client.go)). If you compare it with the plain version, you'll notice the actual client code is very similar. The main
differences are
...
<!---action:show src=zt_echo_client.go highlight=go-->
...
```action:zt
zt edge create identity user founder-laptop -o founder-laptop.jwt --role-attributes management
```
...
```action:zt
zt edge list identities 'name="founder-laptop"'
```
...
```action:zt
zt edge enroll -j founder-laptop.jwt -o founder-laptop.json
```
...
As you saw, the usual flow for enrolling an application in a zt network has two steps. However, unlike in our approach here, we don’t generally rely on the Ziti CLI to enroll applications and
...
```action:zt failOk=true
zt demo zt-echo-client --identity founder-laptop.json trees are tall
```
...
```action:zt
zt edge policy-advisor identities -q founder-laptop echo
```
...
```action:zt
zt edge create service-policy echo-clients Dial --service-roles '@echo' --identity-roles '#management'
```

Let’s check and make sure it’s there:

```action:zt
 zt edge list service-policies 'name="echo-clients"'
```
...
```action:zt templatize=true
zt edge create edge-router-policy echo-clients --edge-router-roles '@${edgeRouterName}' --identity-roles '#management'
```

Taking a look, we should see now see our new edge router policy in place.

```action:zt
zt edge list edge-router-policies 'name="echo-clients"'
```
...
```action:zt
zt edge create service-edge-router-policy echo --edge-router-roles '#all' --service-roles '@echo'
```
...
Before configuring and running our zt-embedded server, we're going to stop the plain server and remove any related configuration.
...
```action:zt colorStdOut=false failOk=true
zt demo zt-echo-client --identity founder-laptop.json trees are tall
```
...
```action:zt
zt edge delete terminators where 'service.name="echo"'
``` 
...
```action:zt
zt edge create identity service echo-server -o echo-server.jwt --role-attributes echo-server
```

We’ll then enroll the server identity.

```action:zt
zt edge enroll -j echo-server.jwt -o echo-server.json
```

## Ziti Server Code

If you look at the server code, viewable [here](../zt_echo_server.go), you should notice very few changes from the plain echo server. The actual request processing code is unchanged.

<!---action:show src=zt_echo_server.go highlight=go-->

Let’s try running it and see what happens.

```action:zt colorStdOut=false failOk=true
zt demo zt-echo-server --identity echo-server.json
```
...
```action:zt
zt edge policy-advisor identities -q echo-server echo
```
...
```action:zt
zt edge create service-policy echo-servers Bind --service-roles '@echo' --identity-roles '#echo-server'
```

We now have two service policies, one for clients and one for hosts

```action:zt
zt edge list service-policies 'name contains "echo"'
```
...
```action:zt templatize=true
zt edge create edge-router-policy echo-servers --edge-router-roles '@${edgeRouterName}' --identity-roles '#echo-server'
```
...
```action:run-zt-echo-server
zt demo zt-echo-server
```
...
There’s a terminator present, which looks quite different from the one we created manually. This terminator was created dynamically when the zt-echo-server application bound the service. Client
