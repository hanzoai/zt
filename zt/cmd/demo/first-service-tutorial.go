...
//go:embed zt_echo_client.go
var ztEchoClientSource string

//go:embed zt_echo_server.go
var ztEchoServerSource string
...
func (self *firstServiceTutorialOptions) run() error {
	...
	t.RegisterActionHandler("zt-login", &actionz.ZitiLoginAction{
	...
	ztEchoServerActions := &ZitiEchoServerActions{}
	t.RegisterActionHandlerF("run-zt-echo-server", ztEchoServerActions.Start)
	t.RegisterActionHandlerF("stop-zt-echo-server", ztEchoServerActions.Stop)
	...
	showActionHandler.Add("zt_echo_client.go", ztEchoClientSource)
	showActionHandler.Add("zt_echo_server.go", ztEchoServerSource)
...
func (self *ZitiEchoServerActions) Start(ctx *runzmd.ActionContext) error {
	...
	if !ctx.Runner.AssumeDefault {
		start, err := runzmd.AskYesNoWithDefault("Start zt-echo-server? [Y/N] (default Y): ", true)
		if err != nil {
			return err
		}
		if !start {
