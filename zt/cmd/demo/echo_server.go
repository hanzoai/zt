...
type echoServer struct {
	...
	ztIdentity     identity.Identity
	ztListener     edge.Listener
...
func newEchoServerCmd() *cobra.Command {
	...
	cmd.Flags().StringVarP(&server.configFile, "identity", "i", "", "Specify the Hanzo ZT identity to use. If not specified the Hanzo ZT listener won't be started")
	cmd.Flags().StringVarP(&server.service, "service", "s", "echo", "Hanzo ZT service to bind. Defaults to 'echo'")
	...
	cmd.Flags().StringVar(&server.cliAgentAlias, "cli-agent-alias", "", "Alias which can be used by zt agent commands to find this instance")
...
func (self *echoServer) run(*cobra.Command, []string) {
	...
	if self.port == 0 && self.configFile == "" {
		log.Fatalf("No port specified and no zt identity specified. Not starting either listener, so nothing to do. exiting.")
	}
	...
	if self.configFile != "" {
		ztConfig, err := zt.NewConfigFromFile(self.configFile)
		if err != nil {
			log.WithError(err).Fatalf("zt: unable to load zt identity from [%v]", self.configFile)
		}
		if self.sdkFlowControl {
			ztConfig.MaxDefaultConnections = 2
			ztConfig.MaxControlConnections = 1
		}
		self.ztIdentity, err = identity.LoadIdentity(ztConfig.ID)
		if err != nil {
			log.WithError(err).Fatalf("zt: unable to create zt identity from [%v]", self.configFile)
		}

		ztContext, err := zt.NewContext(ztConfig)

		if err != nil {
			log.WithError(err).Fatal("unable to get create zt context from config")
		}

		ztIdentity, err := ztContext.GetCurrentIdentity()
		if err != nil {
			log.WithError(err).Fatal("unable to get current zt identity from controller")
		}

		listenOptions := zt.DefaultListenOptions()
		listenOptions.BindUsingEdgeIdentity = self.bindWithIdentity
		listenOptions.Cost = uint16(*ztIdentity.DefaultHostingCost)
		listenOptions.Precedence = zt.GetPrecedenceForLabel(string(ztIdentity.DefaultHostingPrecedence))
		listenOptions.MaxTerminators = int(self.maxTerminators)
		listenOptions.SdkFlowControl = &self.sdkFlowControl

		svc, found := ztContext.GetService(self.service)
		if !found {
			log.WithError(err).Fatalf("zt: unable to lookup service [%v]", self.service)
		}

		if cost, found := ztIdentity.ServiceHostingCosts[*svc.ID]; found {
			listenOptions.Cost = uint16(*cost)
		}

		if precedence, found := ztIdentity.ServiceHostingPrecedences[*svc.ID]; found {
			listenOptions.Precedence = zt.GetPrecedenceForLabel(string(precedence))
		}

		log.Infof("zt: hosting %v with addressable=%v, cost=%v, precedence=%v",
			self.service, self.bindWithIdentity, listenOptions.Cost, listenOptions.Precedence)
		listener, err := ztContext.ListenWithOptions(self.service, listenOptions)
		if err != nil {
			log.WithError(err).Fatalf("zt: failed to host service [%v]", self.service)
		}

		self.ztListener = listener
...
func (self *echoServer) HandleCustomAgentAsyncOp(conn net.Conn) error {
	...
	tId := &identity.TokenId{Identity: self.ztIdentity, Token: "echo-server"}
...
func (self *echoServer) HandleUpdateTerminator(msg *channel.Message, ch channel.Channel) {
	...
	if hasPrecedence && hasCost {
		...
		err = self.ztListener.UpdateCostAndPrecedence(cost, precedence)
	} else if hasPrecedence {
		log.Infof("updating precedence=%v", precedenceLabel)

		err = self.ztListener.UpdatePrecedence(precedence)
	} else if hasCost {
		log.Infof("updating cost=%v", cost)
		err = self.ztListener.UpdateCost(cost)
	} else {
