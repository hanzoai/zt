...
zt edge update identity ${entityName} --role-attributes not-hosting
```

## Create the echo-host config

```action:zt-create-config name=echo-host type=host.v1
{
    "address" : "localhost",
    "port" : 1234,
    "protocol" : "tcp",
...
zt edge create identity echo-host-2 -a echo,echo-host -o echo-host-2.jwt
zt edge enroll --rm echo-host-2.jwt
```
...
```
zt demo echo-server -p 1234 --health-check-addr 0.0.0.0:2234
zt demo echo-server -p 1235 --health-check-addr 0.0.0.0:2235
```
