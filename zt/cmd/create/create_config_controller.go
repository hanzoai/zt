...
`)
	...
	createConfigControllerExample = templates.Examples(`
		...
		zt create config controller --ctrlPort 6262

		# Print the controller config to the console
		zt create config controller --output stdout

		# Print the controller config to a file
		zt create config controller --output <path to file>/<filename>.yaml

		# Create the controller config with an edge router enrollment duration of 2 hours
		zt create config controller --identityEnrollmentDuration 2h
		# OR
		zt create config controller --identityEnrollmentDuration 120m
	`)
)
