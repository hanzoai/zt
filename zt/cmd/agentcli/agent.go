...
func NewAgentCmd(p common.OptionsProvider) *cobra.Command {
	agentCmd := &cobra.Command{
		Use:   "agent",
		Short: "Interact with zt processes using the the IPC agent",
	}
	...
	ctrlCmd := &cobra.Command{
		Use:     "controller",
		Aliases: []string{"c"},
		Short:   "Interact with a zt controller process using the IPC agent",
	}
	...
	routerCmd := &cobra.Command{
		Use:     "router",
		Aliases: []string{"r"},
		Short:   "Interact with a zt router process using the IPC agent",
	}
...
func (self *AgentOptions) GetProcess() (*agent.Process, error) {
	...
	if len(result) == 0 {
		return nil, errors.New("no processes found matching filter, use 'zt agent list' to list candidates")
	}
	...
	if len(result) > 1 {
		...
		return nil, errors.Errorf("too many processes found matching filter: [%v] use 'zt agent list' for more information", strings.Join(pids, " "))
	}
