...
func (self *dbStrategy) ProcessDbModel(tx *bbolt.Tx, m *model.Model, builder *ztlab.ZitiDbBuilder) error {
...
func (self *dbStrategy) loadErMap(csv string) error {
	...
	for sc.Scan() {
		...
		if len(parts) >= 2 {
			routerId := parts[0]
			nfVersion := parts[1]
			ztVersion := nfZitiVersionMap[nfVersion]
			self.routerMappings[routerId] = ztVersion
		}
	}
	return nil
}
...
func (self *dbStrategy) ProcessEdgeRouters(tx *bbolt.Tx, m *model.Model, builder *ztlab.ZitiDbBuilder) error {
...
func (self *dbStrategy) CreateEnrollIdentitiesAction(tx *bbolt.Tx, m *model.Model, builder *ztlab.ZitiDbBuilder) error {
...
var m = &model.Model{
	...
	StructureFactories: []model.Factory{
		&ztlab.ZitiDbBuilder{Strategy: dbStrategyInstance},
	},
