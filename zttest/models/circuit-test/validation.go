...
import (
	...
	loop4Pb "github.com/hanzozt/zt/zttest/zt-traffic-test/loop4/pb"
...
func RunSimScenarios(run model.Run, services *ztLibOps.SimServices) error {
	...
	sims := run.GetModel().FilterComponents(".loop-client", func(c *model.Component) bool {
		t, ok := c.Type.(*ztlab.Loop4SimType)
		return ok && t.Mode == ztlab.Loop4RemoteControlled
	})
...
func validateCircuitsForCtrlOnce(id string, clients *ztrest.Clients, first bool) (int, error) {
