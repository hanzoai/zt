	...
	StructureFactories: []model.Factory{
		...
		model.FactoryFunc(func(m *model.Model) error {
			...
			err := m.ForEachComponent(".client-router", 1, func(c *model.Component) error {
				c.Type.(*ztlab.RouterType).Version = ClientRoutersVersion
				return nil
			})
			...
			err = m.ForEachComponent(".ctrl", 1, func(c *model.Component) error {
				c.Type.(*ztlab.ControllerType).Version = DefaultVersion
				return nil
			})
			...
			return m.ForEachComponent(".host-router", 1, func(c *model.Component) error {
				c.Type.(*ztlab.RouterType).Version = HostRoutersVersion
				return nil
			})
		}),
		model.FactoryFunc(func(m *model.Model) error {
		...
		model.FactoryFunc(func(m *model.Model) error {
			for _, host := range m.SelectHosts("*") {
				...
				for _, component := range host.Components {
					if rc, ok := component.Type.(*ztlab.Loop4SimType); ok && rc.Mode == ztlab.Loop4RemoteControlled {
					...
					Components: model.Components{
						...
						"loop-client-ert": {
							...
							Type: &ztlab.Loop4SimType{
								Mode: ztlab.Loop4RemoteControlled,
							},
						},
						"tcpdump-ert": {
							Scope: model.Scope{Tags: model.Tags{"tcpdump", "client"}},
