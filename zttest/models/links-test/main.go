...
var m = &model.Model{
	...
	Factories: []model.Factory{
		model.FactoryFunc(func(m *model.Model) error {
			return m.ForEachComponent("component.router", 1, func(c *model.Component) error {
				if routerType, ok := c.Type.(*ztlab.RouterType); ok {
					clone := *routerType
					c.Type = &clone
	...
	Actions: model.ActionBinders{
		"bootstrap": model.ActionBinder(func(m *model.Model) model.Action {
			...
			workflow.AddAction(edge.InitEdgeRoutersWithClients(models.RouterTag, 50, func(run model.Run) (*ztrest.Clients, error) {
				if ctrl1 == nil {
