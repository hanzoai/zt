...
var m = &model.Model{
				...
				"zrokFront": {
					...
					Components: model.Components{
						"zrokFront": {
							...
							Type: &ztlab.ZrokFrontendType{
								Version: TargetZrokVersion,
								DNS:     "paul.demo.hanzozt.org",
							},
						},
					},
				},
	...
	Actions: model.ActionBinders{
		"bootstrap": model.ActionBinder(func(m *model.Model) model.Action {
			...
			workflow.AddAction(component.ExecF("#zrokCtrl", (*ztlab.ZrokControllerType).Init))
			workflow.AddAction(component.ExecF("#zrokCtrl", (*ztlab.ZrokControllerType).PreCreateAccounts))
			workflow.AddAction(component.Start("#zrokCtrl"))
			workflow.AddAction(semaphore.Sleep(2 * time.Second))

			workflow.AddAction(component.ExecF("#zrokFront", (*ztlab.ZrokFrontendType).Init))
			workflow.AddAction(component.Start("#zrokFront"))
			workflow.AddAction(semaphore.Sleep(2 * time.Second))

			workflow.AddAction(component.ExecInParallelF(".client", 10, (*ztlab.ZrokLoopTestType).Init))
			return workflow
		}),
