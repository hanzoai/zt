...
func (self *bootstrapAction) bind(m *model.Model) model.Action {
	...
	for _, router := range m.SelectComponents(models.RouterTag) {
		...
		workflow.AddAction(ztlib_actions.Fabric("create", "router", filepath.Join(model.PkiBuild(), cert)))
	}
	...
	for _, h := range m.SelectHosts("*") {
		workflow.AddAction(host.Exec(h, fmt.Sprintf("mkdir -p /home/%s/.zt", sshUsername)))
		workflow.AddAction(host.Exec(h, fmt.Sprintf("rm -f /home/%s/.zt/identities.yml", sshUsername)))
		workflow.AddAction(host.Exec(h, fmt.Sprintf("ln -s /home/%s/fablab/cfg/remote_identities.yml /home/%s/.zt/identities.yml", sshUsername, sshUsername)))
	}
...
func (_ *bootstrapAction) createServiceActions(m *model.Model) ([]model.Action, error) {
	...
	for _, h := range hosts {
		serviceActions = append(serviceActions, ztlib_actions.Fabric("create", "service", h.GetId()))
		serviceActions = append(serviceActions, ztlib_actions.Fabric("create", "terminator", h.GetId(), router.Id, "tcp:"+h.PrivateIp+":8171"))
	}
