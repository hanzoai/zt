...
import (
	...
	aws_ssh_keys0 "github.com/hanzozt/fablab/kernel/lib/runlevel/0_infrastructure/aws_ssh_key"
	semaphore0 "github.com/hanzozt/fablab/kernel/lib/runlevel/0_infrastructure/semaphore"
	terraform0 "github.com/hanzozt/fablab/kernel/lib/runlevel/0_infrastructure/terraform"
	"github.com/hanzozt/fablab/kernel/lib/runlevel/1_configuration/config"
	...
	aws_ssh_keys6 "github.com/hanzozt/fablab/kernel/lib/runlevel/6_disposal/aws_ssh_key"
	terraform6 "github.com/hanzozt/fablab/kernel/lib/runlevel/6_disposal/terraform"
	"github.com/hanzozt/fablab/kernel/model"
	"github.com/hanzozt/zt/zttest/ztlab/models"
	ztlab_5_operation "github.com/hanzozt/zt/zttest/ztlab/runlevel/5_operation"
)
...
func (self *stageFactory) Build(m *model.Model) error {
	...
	// 4. The actual binaries, including the controller, router, zt-fabric CLI and the zt-fabric-test tool
	...
	// 4. Create config files that the zt-fabric-test loop tool can use to connect and send data
...
func (self *stageFactory) addOperationStages(m *model.Model) error {
	...
	m.AddOperatingStage(ztlab_5_operation.Mesh(phase.GetCloser()))
	//m.AddOperatingStage(ztlab_5_operation.Metrics(phase.GetCloser()))
...
func (_ *stageFactory) addListenerStages(m *model.Model) error {
	...
	for _, host := range hosts {
		m.AddOperatingStage(ztlab_5_operation.LoopListener(host, nil, "tcp:0.0.0.0:8171"))
	}
...
func (_ *stageFactory) dialers(m *model.Model, phase fablib_5_operation.Phase) error {
	...
	for _, host := range hosts {
		m.AddOperatingStage(ztlab_5_operation.LoopDialer(host, "10-ambient.loop2.yml", endpoint, phase.AddJoiner()))
	}
