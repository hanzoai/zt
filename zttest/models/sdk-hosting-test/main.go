...
import (
	...
	errUtil "github.com/hanzozt/zt/v2/zt/util"
...
var m = &model.Model{
	...
	StructureFactories: []model.Factory{
		model.FactoryFunc(func(m *model.Model) error {
			...
			err = m.ForEachComponent(".host", 1, func(c *model.Component) error {
				if useZde {
					c.Type = &ztlab.ZitiEdgeTunnelType{
						Version: "1.9.5",
						Mode:    ztlab.ZitiEdgeTunnelModeHost,
					}
				} else {
	...
	Actions: model.ActionBinders{
		"createBaseModel": model.ActionBinder(func(m *model.Model) model.Action {
			...
			workflow.AddAction(ztlibActions.Edge("create", "edge-router-policy", "all", "--edge-router-roles", "#all", "--identity-roles", "#all"))
			workflow.AddAction(ztlibActions.Edge("create", "service-edge-router-policy", "all", "--service-roles", "#all", "--edge-router-roles", "#all"))
			...
			workflow.AddAction(ztlibActions.Edge("create", "config", "host-config", "host.v1", hostConfig))
