...
func (a *bootstrapAction) bind(m *model.Model) model.Action {
	...
	// Create Configs
	workflow.AddAction(ztlib_actions.Edge("create", "config", "iperf-server", "host.v1", `
					{
							"address" : "localhost",
							"port" : 7001,
							"protocol" : "tcp"
	...
	workflow.AddAction(semaphore.Sleep(2 * time.Second))
	workflow.AddAction(ztlib_actions.Edge("create", "config", "iperf-intercept", "intercept.v1", `
		{
