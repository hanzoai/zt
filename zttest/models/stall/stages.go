...
import (
	...
	ztlib_5_operation "github.com/hanzozt/zt/zttest/ztlab/runlevel/5_operation"
)
...
func (self *stageFactory) Build(m *model.Model) error {
	...
	clientMetrics := ztlib_5_operation.NewSimServices(func(s string) string {
		return "component#" + s
	})
	...
	//m.AddOperatingStage(ztlib_5_operation.Mesh(runPhase.GetCloser()))
	m.AddOperatingStage(ztlib_5_operation.ModelMetricsWithIdMapper(runPhase.GetCloser(), func(id string) string {
		if id == "ctrl" {
			return "#ctrl"
		}
	...
	m.AddOperatingStage(ztlib_5_operation.CircuitMetrics(5*time.Second, runPhase.GetCloser(), func(id string) string {
		id = strings.ReplaceAll(id, ".", ":")
		return "component.edgeId:" + id
	}))
...
func (_ *stageFactory) listeners(m *model.Model) error {
	...
	stage := ztlib_5_operation.Loop3Listener(c.GetHost(), nil, "tcp:0.0.0.0:8171",
...
func (_ *stageFactory) dialers(m *model.Model, phase fablib_5_operation.Phase) error {
	...
	remoteConfigFile := fmt.Sprintf("/home/%v/fablab/cfg/%v.json", m.MustVariable("credentials.ssh.username"), c.Id)
	stage := ztlib_5_operation.Loop3Dialer(c.GetHost(), c.Id, "tcp:test.service:8171", phase.AddJoiner(), "--config-file", remoteConfigFile, "-d")
	m.AddOperatingStage(stage)
