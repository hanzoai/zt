...
var m = &model.Model{
	...
	StructureFactories: []model.Factory{
		model.FactoryFunc(func(m *model.Model) error {
			...
			err = m.ForEachComponent(".host", 1, func(c *model.Component) error {
				c.Type.(*ztlab.ZitiTunnelType).Mode = ztlab.ZitiTunnelModeHost
				return nil
			})
	...
	Actions: model.ActionBinders{
		"bootstrap": model.ActionBinder(func(m *model.Model) model.Action {
			...
			workflow.AddAction(ztlib_actions.Edge("create", "edge-router-policy", "all", "--edge-router-roles", "#all", "--identity-roles", "#all"))
			workflow.AddAction(ztlib_actions.Edge("create", "service-edge-router-policy", "all", "--service-roles", "#all", "--edge-router-roles", "#all"))

			workflow.AddAction(ztlib_actions.Edge("create", "config", "host-config", "host.v1", `
				{
					"address" : "localhost",
					"port" : 8080,
					"protocol" : "tcp"
			...
			workflow.AddAction(ztlib_actions.Edge("create", "service", "test-svc", "-c", "host-config"))
			workflow.AddAction(ztlib_actions.Edge("create", "service-policy", "test-svc-policy", "Bind",
