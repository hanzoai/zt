...
func validateTerminatorsForCtrl(c *model.Component, deadline time.Time) error {
	...
	var clients *ztrest.Clients
	loginStart := time.Now()
	for {
		var err error
		clients, err = ztrest.NewManagementClients(edgeApiBaseUrl)
		if err != nil {
...
func getTerminatorCount(clients *ztrest.Clients) (int64, error) {
...
func validateRouterErtTerminators(id string, clients *ztrest.Clients) (int, error) {
