...
import (
	...
	"github.com/hanzozt/zt/v2/common/outputz"
	trafficMetrics "github.com/hanzozt/zt/zttest/zt-traffic-test/metrics"
	"github.com/hanzozt/zt/zttest/zt-traffic-test/subcmd"
...
func NewSim() *Sim {
	return &Sim{
		sdkClients:  make(map[string]zt.Context),
...
func (sim *Sim) InitConnectors() error {
	...
	for name, connector := range sim.scenario.ConnectorConfigs {
		if connector.SdkOptions != nil {
			...
			cfg, err := zt.NewConfigFromFile(connector.SdkOptions.IdentityFile)
			if err != nil {
				return fmt.Errorf("for connector '%s', unable to load zt identity config file '%s' (%w)",
			...
			if err != nil {
				return fmt.Errorf("for connector '%s', unable to create zt sdk context for identity file '%s' (%w)",
					name, connector.SdkOptions.IdentityFile, err)
			}
			...
			if connector.SdkOptions.ReportSdkMetrics {
				sim.sdkMetricsStartHooks = append(sim.sdkMetricsStartHooks, func(metricsCtx zt.Context) error {
			...
			if addr.Type() != "tcp" {
				if _, id, err = dotzt.LoadIdentity(connector.TransportOptions.IdentityFile); err != nil {
...
func (sim *Sim) ListenSdk(client zt.Context, wf *Workload, useSdkFlowControl bool) (net.Listener, error) {
...
func (sim *Sim) StartSdkMetricsReporter(registry metrics.Registry, ctx zt.Context) error {
