...
func NewZitiReporter(cfg *ZitiReporterConfig) (*ZitiReporter, error) {
	if cfg.Registry == nil {
		return nil, errors.New("missing metrics registry for zt metrics reporter")
	}

	if cfg.ClientId == "" {
		return nil, errors.New("missing client id for zt metrics reporter")
	}

	if cfg.ServiceName == "" {
		return nil, errors.New("missing service name for zt metrics reporter")
	}

	client := cfg.Client
	if client == nil {
		if cfg.IdentityConfigFile != "" {
			sdkConfig, err := zt.NewConfigFromFile(cfg.IdentityConfigFile)
			if err != nil {
				return nil, fmt.Errorf("unable to load zt identity for metrics reporter (%w)", err)
			}
			client, err = zt.NewContext(sdkConfig)
			if err != nil {
				return nil, fmt.Errorf("unable to create zt sdk client for metrics reporter (%w)", err)
			}
		} else {
			return nil, errors.New("no zt client and no config file provided for metris reporter")
		}
	}
