...
import (
	...
	"github.com/hanzozt/zt/zttest/zt-traffic-test/loop2/pb"
...
func (cmd *dialerCmd) connect() net.Conn {
	...
	if strings.HasPrefix(cmd.endpoint, "edge:") {
		...
		if cmd.edgeConfigFile != "" {
			...
			if err != nil {
				log.Fatalf("failed to load zt context from configuration: %v", err)
			}
		} else {
		...
		_, id, err := dotzt.LoadIdentity(cmd.identity)
		if err != nil {
			panic(err)
		}
