...
func newDialerCmd() *dialerCmd {
	...
	flags.StringVarP(&result.identity, "identity", "i", "default", ".zt/identities.yml name")
...
func (cmd *dialerCmd) connect() net.Conn {
	...
	if strings.HasPrefix(cmd.endpoint, "edge:") {
		...
		if cmd.edgeConfigFile != "" {
			ztCfg, err := zt.NewConfigFromFile(cmd.edgeConfigFile)
			if err != nil {
				log.Fatalf("failed to load zt configuration from %s: %v", cmd.edgeConfigFile, err)
			}
			context, err = zt.NewContext(ztCfg)

			if err != nil {
				log.Fatalf("failed to load zt context from config: %v", err)
			}
		} else {
		...
		conn, err = context.DialWithOptions(service, &zt.DialOptions{
		...
		if endpoint.Type() != "tcp" || !cmd.direct {
			if _, id, err = dotzt.LoadIdentity(cmd.identity); err != nil {
				panic(err)
			}
		}
