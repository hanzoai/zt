...
func (r *ztMetricsReporter) run(reportInterval time.Duration) {
	log := pfxlog.Logger()
	...
	var client zt.Context
	if r.configFile != "" {
		sdkConfig, err := zt.NewConfigFromFile(r.configFile)
		if err != nil {
			panic(err)
		}
		client, err = zt.NewContext(sdkConfig)
...
func (r *ztMetricsReporter) createMetricsEvent() *mgmt_pb.StreamMetricsEvent {
...
func (r *ztMetricsReporter) addMeterToEvent(event *mgmt_pb.StreamMetricsEvent, name string, metric metrics.Meter) {
...
func (r *ztMetricsReporter) addHistogramToEvent(event *mgmt_pb.StreamMetricsEvent, name string, metric metrics.Histogram) {
	r.addIntMetric(event, name, "count", metric.Count())
...
func (r *ztMetricsReporter) addTimerToEvent(event *mgmt_pb.StreamMetricsEvent, name string, metric metrics.Timer) {
	r.addIntMetric(event, name, "count", metric.Count())
	r.addFloatMetric(event, name, "mean_rate", metric.RateMean())
	r.addFloatMetric(event, name, "m1_rate", metric.Rate1())
	r.addFloatMetric(event, name, "m5_rate", metric.Rate5())
...
func (r *ztMetricsReporter) addIntMetric(event *mgmt_pb.StreamMetricsEvent, base, name string, val int64) {
	fullName := base + "." + name
	event.IntMetrics[fullName] = val
	event.MetricGroup[fullName] = base
}

func (r *ztMetricsReporter) addFloatMetric(event *mgmt_pb.StreamMetricsEvent, base, name string, val float64) {
	fullName := base + "." + name
