...
func (self *loopListener) Execute(run model.Run) error {
	...
	if err := libssh.RemoteKill(ssh, fmt.Sprintf("zt-fabric-test %v listener", self.subcmd)); err != nil {
...
func (self *loopListener) run(run model.Run) {
	...
	listenerCmd := fmt.Sprintf("/home/%s/fablab/bin/zt-fabric-test %v listener -b %v %v >> %s 2>&1",
