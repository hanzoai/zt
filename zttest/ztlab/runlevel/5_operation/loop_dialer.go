...
func (self *loopDialer) Execute(run model.Run) error {
	...
	if err := libssh.RemoteKill(ssh, fmt.Sprintf("zt-fabric-test %v dialer", self.subcmd)); err != nil {
...
func (self *loopDialer) run(ctx model.Run) {
	...
	dialerCmd := fmt.Sprintf("/home/%s/fablab/bin/zt-fabric-test %v dialer /home/%s/fablab/cfg/%s -e %s -s %s %s >> %s 2>&1",
