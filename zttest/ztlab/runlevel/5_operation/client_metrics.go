...
package ztlib_runlevel_5_operation
...
import (
	...
	"github.com/hanzozt/zt/zttest/zt-traffic-test/loop4"
...
func (self *SimServices) SetupSimControllerIdentity(run model.Run) error {
	if err := ztlibActions.EdgeExec(run.GetModel(), "delete", "identity", SimControllerName); err != nil {
		return err
	}

	jwtFilePath := run.GetLabel().GetFilePath("sim-controller.jwt")
	if err := ztlibActions.EdgeExec(run.GetModel(), "create", "identity", SimControllerName, "-a", "metrics-host,sim-services-host", "-o", jwtFilePath); err != nil {
		return err
	}
...
func (self *SimServices) GetZitiContext(run model.Run) (zt.Context, error) {
	self.lock.Lock()
	defer self.lock.Unlock()
	if self.ztContext == nil {
		identityConfigPath := run.GetLabel().GetFilePath("sim-controller.json")
		pfxlog.Logger().Infof("loading zt config from [%s]", identityConfigPath)
		cfg, err := zt.NewConfigFromFile(identityConfigPath)
		if err != nil {
			return nil, err
		}
		pfxlog.Logger().Infof("loading zt context from [%s]", identityConfigPath)
		context, err := zt.NewContext(cfg)
		if err != nil {
			return nil, err
		}
		self.ztContext = context
	}

	return self.ztContext, nil
}
...
func (self *SimServices) CollectSimMetrics(run model.Run, service string) error {
	...
	go func() {
		pfxlog.Logger().Info("zt client metrics listener started")
		for {
...
func (self *SimServices) GetSimController(run model.Run, service string, callback loop4.ControllerCallback) (*loop4.RemoteController, error) {
	ztContext, err := self.GetZitiContext(run)
	if err != nil {
		return nil, err
	}
	...
	if self.remoteController == nil {
		simControl := loop4.NewRemoteController(ztContext, callback)
