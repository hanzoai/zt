...
func getZitiProcessFilter(c *model.Component, ztType string) func(string) bool {
	return func(s string) bool {
		matches := strings.Contains(s, "zt") &&
			strings.Contains(s, ztType) &&
...
func startZitiComponent(c *model.Component, ztType string, version string, configName string, extraArgs string) error {
	...
	useSudo := ""
	if ztType == "tunnel" || c.HasTag("tunneler") {
		useSudo = "sudo"
	}

	serviceCmd := fmt.Sprintf("nohup %s %s %s run %s --cli-agent-alias %s --log-formatter json %s > %s 2>&1 &",
		useSudo, binaryPath, ztType, extraArgs, c.Id, configPath, logsPath)
...
func GetZitiBinaryPath(c *model.Component, version string) string {
	return getBinaryPath(c, "zt", version)
}
...
func reEnrollIdentity(run model.Run, c *model.Component, ztBinaryPath string, configPath string) error {
	if err := ztlib_actions.EdgeExec(run.GetModel(), "delete", "authenticator", "where", fmt.Sprintf("identity=\"%v\"", c.Id)); err != nil {
		return err
	}

	if err := ztlib_actions.EdgeExec(run.GetModel(), "delete", "enrollment", "where", fmt.Sprintf("identity=\"%v\"", c.Id)); err != nil {
		return err
	}
	...
	if err := ztlib_actions.EdgeExec(c.GetModel(), args...); err != nil {
		return err
	}
	...
	cmd := fmt.Sprintf(tmpl, configDir, ztBinaryPath, remoteJwt, configPath, c.Id)

	return c.GetHost().ExecLogOnlyOnError(cmd)
}

func setupDnsForTunneler(c *model.Component) error {
	key := "zt_tunnel.resolve_setup_done"
