...
func (self *ZitiEdgeTunnelType) StageFiles(r model.Run, c *model.Component) error {
	if err := stagezt.StageZitiEdgeTunnelOnce(r, c, self.Version, self.LocalPath); err != nil {
		return err
	}
	return stagezt.StageZitiOnce(r, c, self.ZitiVersion, self.LocalPath)
}
...
func (self *ZitiEdgeTunnelType) Start(_ model.Run, c *model.Component) error {
	...
	if self.Mode == ZitiEdgeTunnelModeDefault {
		...
		return fmt.Errorf("unsupported zt-edge-tunnel mode: %v", self.Mode)
	}
