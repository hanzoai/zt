...
func (action *initEdgeRoutersAction) Execute(run model.Run) error {
	if err := ztlib_actions.EdgeExec(run.GetModel(), "delete", "edge-router", "where", "true"); err != nil {
		pfxlog.Logger().WithError(err).Warn("unable to delete routers")
	}

	return component.ExecInParallel(action.componentSpec, action.concurrency, ztlab.RouterActionsCreateAndEnroll).Execute(run)
}
...
type clientsInitializer func(run model.Run) (*ztrest.Clients, error)
...
func (action *initEdgeRoutersWithClientsAction) Execute(run model.Run) error {
	...
	for _, c := range run.GetModel().SelectComponents(action.componentSpec) {
		if v, ok := c.Type.(*ztlab.RouterType); ok {
