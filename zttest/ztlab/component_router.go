...
func (self *RouterType) CreateAndEnroll(run model.Run, c *model.Component) error {
	...
	if isTunneler {
		attempt := 1
		for {
			if err := ztlib_actions.EdgeExec(c.GetModel(), "update", "identity", c.Id, "-a", attributes, "--timeout", strconv.Itoa(60)); err != nil {
				if attempt > 4 {
					return err
				}
				attempt++
...
func (self *RouterType) CreateAndEnrollTasks(run model.Run, c *model.Component, client *ztrest.Clients) []parallel.LabeledTask {
...
func (self *RouterType) ReEnroll(run model.Run, c *model.Component) error {
	...
	tmpl := "set -o pipefail; %s router enroll /home/ubuntu/fablab/cfg/%s -j %s 2>&1 | tee /home/ubuntu/logs/%s.router.enroll.log "
	cmd := fmt.Sprintf(tmpl, GetZitiBinaryPath(c, ztVersion), self.GetConfigName(c), remoteJwt, c.Id)
