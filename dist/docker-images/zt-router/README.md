...
The `hanzozt/zt-router` image is thin and is based on the `hanzozt/zt-cli` image, which only provides the `zt`
CLI. This `zt-router` image adds an entrypoint that provides router bootstrapping when `ZITI_BOOTSTRAP=true` and uses
...
wget https://get.hanzozt.dev/dist/docker-images/zt-router/compose.yml
...
```text
# fetch the compose file for the zt-router image
wget -O ./compose.router.yml https://get.hanzozt.dev/dist/docker-images/zt-router/compose.yml
# fetch the router tproxy compose overrides files
wget -O ./compose.tproxy.yml https://get.hanzozt.dev/dist/docker-images/zt-router/compose.override.yml
...
cat <<EOF >>./compose.tproxy.yml
  ...
  tproxy-demo-client:
    image: busybox
    network_mode: service:zt-router
    depends_on:
      zt-router:
...
```text
zt edge update identity quickstart-router \
...
```text
zt edge create edge-router "tproxy-router" \
...
```text
zt edge update identity tproxy-router \
...
ZITI_ROUTER_ADVERTISED_ADDRESS=zt-router \
