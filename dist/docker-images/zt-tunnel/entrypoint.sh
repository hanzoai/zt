...
function alldone() {
    ...
    if [[ "${ZITI_TUNNEL_PID:-}" =~ ^[0-9]+$ ]] && kill -0 "$ZITI_TUNNEL_PID" &>/dev/null; then
        ...
        # let entrypoint script exit after zt tunnel PID
    ...
    while (( ZITI_IDENTITY_WAIT > 0 || ZITI_IDENTITY_WAIT < 0 )); do
        ...
        # look for enrollment token
            ...
            if [[ -n "${JWT_FILE:-}" ]]; then
                ...
                zt edge enroll --jwt "${JWT_FILE}" --out "${IDENTITY_FILE}" || {
...
echo "INFO: running \"zt tunnel ${TUNNEL_RUN_MODE} ${TUNNEL_OPTS[*]} ${*}\""
zt tunnel "${TUNNEL_RUN_MODE}" "${TUNNEL_OPTS[@]}" "${@}" &
ZITI_TUNNEL_PID=$!
