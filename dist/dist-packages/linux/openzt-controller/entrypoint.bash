...
# zt. The bootstrap() function will exit 0 immediately if ZITI_BOOTSTRAP is not set to true. Otherwise,  it will
...
source "${ZITI_CTRL_BOOTSTRAP_BASH:-/opt/hanzozt/etc/controller/bootstrap.bash}"

# * Linux service ExecStartPre uses 'check' to pre-flight the config and renew certs
# * Container uses 'run' to call bootstrap() and zt
if [[ "${ZITI_BOOTSTRAP:-}" == true && "${1}" =~ run ]]; then
  ...
  if [[ ! -s "${2}" ]]; then
    ...
    loadEnvFiles /opt/hanzozt/etc/controller/bootstrap.env
    issueLeafCerts
    exit
  else
