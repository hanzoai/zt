...
makePki() {
  ...
  # used by "zt pki create server" as DNS SAN
...
issueLeafCerts() {
  ...
  # client cert
  #   use the server key for both client and server certs until "zt create config controller" supports separate keys for
  #   each
...
makeConfig() {
  ...
  # used by "zt create config controller" as advertised address
  if [[ -z "${ZITI_CTRL_ADVERTISED_ADDRESS:-}" ]]; then
  ...
  # set the URI of the edge-client API (uses same TCP port); e.g., ztAPI: ctrl.zt.example.com:1280
  ...
  local -a _command=(zt create config controller --clustered --output "${_config_file}")
...
stashZitiEnv() {
  declare -gA _zt_env_saved=()
  while IFS='=' read -r _key _value; do
    _zt_env_saved["${_key}"]="${_value}"
  done < <(env | grep '^ZITI_')
}

restoreZitiEnv() {
  for _key in "${!_zt_env_saved[@]}"; do
    if [[ -n "${_zt_env_saved[${_key}]}" ]]; then
      printf -v "${_key}" '%s' "${_zt_env_saved[${_key}]}"
    fi
  done
  unset _zt_env_saved _key _value
}
...
exportZitiVars() {
  # make zt vars available in forks
  for line in $(set | grep -e "^ZITI_" | sort); do
    # shellcheck disable=SC2013
    export "${line%=*}"
  done
...
# - Docker: called after controller is running in background, uses zt agent directly
#
# Arguments:
...
clusterInit() {
  ...
  if [[ -n "${_pid}" ]]; then
    ...
    _output="$(nsenter --target "${_pid}" --mount -- \
      zt agent cluster init --pid "${_pid}" "${ZITI_USER}" "${ZITI_PWD}" "${ZITI_USER_NAME}" 2>&1)"
    _rc=$?
    set -o errexit
  else
    # Docker path: call zt agent directly (same namespace)
    echo "DEBUG: using direct zt agent call" >&3
    set +o errexit
    _output="$(zt agent cluster init "${ZITI_USER}" "${ZITI_PWD}" "${ZITI_USER_NAME}" 2>&1)"
    _rc=$?
    set -o errexit
  fi
...
waitForAgent() {
  ...
  while (( _attempts-- )); do
    ...
    set +o errexit
    if [[ -n "${_pid}" ]]; then
      nsenter --target "${_pid}" --mount -- zt agent stats --pid "${_pid}" &>/dev/null
      _rc=$?
    else
      zt agent stats &>/dev/null
      _rc=$?
    fi
    set -o errexit
...
hintLinuxBootstrap() {
  ...
  echo -e "\nProvide a configuration in '${_work_dir}' or generate with:"\
          ...
          "\n* Run 'systemctl enable --now zt-controller.service'"\
          "\n"
}
...
if ! [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  ...
  export ZITI_HOME=/var/lib/private/zt-controller
  SVC_ENV_FILE=/opt/hanzozt/etc/controller/service.env
  BOOT_ENV_FILE=/opt/hanzozt/etc/controller/bootstrap.env
  SVC_FILE=/etc/systemd/system/zt-controller.service.d/override.conf
  ...
  then
    ...
    if [[ "${ZITI_BOOTSTRAP_CLUSTER:-}" == true || "${ZITI_BOOTSTRAP_DATABASE:-}" == true ]]; then
      ...
      if [[ -d /run/systemd/system ]]; then
        ...
        systemctl start zt-controller.service

        # Get the controller PID and wait for agent
        _ctrl_pid="$(systemctl show -p MainPID --value zt-controller.service)"
