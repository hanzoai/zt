...
restoreZitiEnv() {
  for _key in "${!_zt_env_saved[@]}"; do
    if [[ -n "${_zt_env_saved[${_key}]}" && -z "${!_key-}" ]]; then
...
makeConfig() {
  ...
  # used by "zt create config" as controller host
  ...
  if [[ ! -s "${_config_file}" || "${1:-}" == --force ]]; then
    ...
    local -a _command=("zt create config router ${ZITI_ROUTER_TYPE}" \
...
if ! [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  ...
  export ZITI_HOME=/var/lib/private/zt-router
  BOOT_ENV_FILE=/opt/hanzozt/etc/router/bootstrap.env
  SVC_ENV_FILE=/opt/hanzozt/etc/router/service.env
  SVC_FILE=/etc/systemd/system/zt-router.service.d/override.conf
